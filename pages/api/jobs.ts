...
function generatePrompt(input: {
    ...
    prompt += `You are required to generate a JSON array of next job positions that both follows my potential career path and aligns with my resume (name:string, company:string, monthlySalaryLowInDollar:number, monthlySalaryHighInDollar:number, requiredDegree:string, responsibility:string[], requiredSkills:string[], requiredExperience:string[]). Surround keys with a pair of commas.`;
